...
def play_one_game(game_idx, mcts, simulations, num_games):
    ...
    while not state.terminal:
        ...
        game_history.append((feat.clone().detach(), cleared_line))
...
def self_play(model, buffer, device, num_games=10, simulations=100, batch_size=32):
    ...
    for history, final_score in results:
        ...
        for _ in range(multiplier):
            for (feat, cleared_line) in history:
