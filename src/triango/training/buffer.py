...
class ReplayBuffer(Dataset[tuple[torch.Tensor, torch.Tensor, torch.Tensor]]):
    def __init__(self, capacity: int = 250000, elite_ratio: float = 0.1):
        ...
        self.buffer: list[tuple[torch.Tensor, float, float]] = []
        self.elite_buffer: list[tuple[torch.Tensor, float, float]] = []
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if idx < len(self.buffer):
            ...
            state, target_value, target_line_clear = self.elite_buffer[idx - len(self.buffer)]
    ...
    def push(
        self, state_tensor: torch.Tensor, target_value: float, target_line_clear: float
    ) -> None:
    ...
    def push_game(
        self, game_history: list[tuple[torch.Tensor, float, float]], final_score: float
    ) -> None:
        """
        ...
        if final_score >= self.elite_threshold and final_score > 0:
            # Add to elite buffer
            for state, cleared_line, state_score in game_history:
                ...
                self.elite_buffer.append((state, rem_score, cleared_line))
            ...
            for state, cleared_line, state_score in game_history:
                rem_score = max(0.0, float(final_score - state_score))
                self.push(state, rem_score, cleared_line)
